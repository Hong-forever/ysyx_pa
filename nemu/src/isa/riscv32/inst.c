...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = src1 + imm; ftrace_exec(s->pc, s->dnpc, src1, rd, imm, 2); R(rd) = s->pc + 4);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + imm; ftrace_exec(s->pc, s->dnpc, src1, rd, imm, 1); R(rd) = s->pc + 4);
